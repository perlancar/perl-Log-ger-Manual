...
so on. Log::ger also records the importer's package name. When a resetup is
requested (e.g. due to a change of log level or outputs), Log::ger will again
construct logging routines and install them for each importer that has been
recorded, updating the routines.

Actually, instead of installing routines to a package, Log::ger can also target
a hash or an object (which is basically the same as installing to a package, but
a "random" package name will be used for the object).
