...
By default, level filtering is done by Log::ger installing null logging routines
(C<<sub {0}>>) for routine names that are above the current level
(C<<$Log::ger::Current_Level>>). For example, the default level is warn (30) so
...
(20), C<log_fatal> (10) will get installed the logging routine described above.
