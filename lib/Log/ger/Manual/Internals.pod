...
C<$lnum> is numeric level, and C<$lname> is string level. The layouter must
return the laid-out message (usually a string).
...
Used to create multiple-level "log" routines.

It should return:

 [\&create_logml_routine, $flow_control]

C<&create_logml_routine> will be called with:

 (\%init_args, $level, $msg)

for each routine specified in the C<logml_subs> (or C<logml_methods>) in the
routine names (see documentation on C<create_routine_names> phase). Extra
arguments received by hooks: C<name> (routine name).
