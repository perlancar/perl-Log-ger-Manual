...
A way to, or an attribute by which one can filter log messages. Category
typically maps to Perl packages from which a log message is produced, but
...
(executed first) and 100 means very low (executed last). Hooks which do not care
about the order in which it is executed with regard to other hooks should set
its priority to 50 (normal).
...
The layouter must return the laid-out message (usually a string).
