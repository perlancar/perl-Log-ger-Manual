package Log::ger::Manual::Tutorial::02;
...
These statements logger subroutines, one for each level. They accept a string:
...
In addition to the logger subroutines, there are also the level detection
