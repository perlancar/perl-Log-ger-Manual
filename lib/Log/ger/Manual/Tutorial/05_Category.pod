package Log::ger::Manual::Tutorial::05;
...
category C<Foo> to a separate file C</tmp/foo.log> while the rest go to
C</tmp/app.log>, you can do something like this:

 use Log::ger::Output Composite => (
     outputs => {
         File => [
             {
                 conf => {path=>'/tmp/app.log'},
